...
const templatePath = path.join(__dirname, "contracts", "strategy-v9.json");
const outputPath = path.join(__dirname, "contracts", "strategy-v10.json");
...
if (head !== sourceCommit) throw new Error("create v10 only from the exact clean strategy source commit");
if (fs.existsSync(outputPath)) throw new Error("strategy-v10.json already exists");
...
contract.contractVersion = "v10";
contract.name = "A股情绪周期统一决策回测V10可插拔数据源基线";
contract.strategyBaseline.baselineId = "unified-decision-v3-factors-v6-minute-sell-v10-provider-v1";
...
providerSourceFiles.forEach(([sourcePath, scope]) => sourceEntries.set(sourcePath, { path: sourcePath, scope }));
