...
const templatePath = path.join(__dirname, "contracts", "strategy-v8.json");
const outputPath = path.join(__dirname, "contracts", "strategy-v9.json");
...
if (head !== sourceCommit) {
  throw new Error("create v9 only from the exact clean strategy source commit");
}
if (fs.existsSync(outputPath)) throw new Error("strategy-v9.json already exists");

const contract = JSON.parse(fs.readFileSync(templatePath, "utf8"));
contract.contractVersion = "v9";
contract.name = "A股情绪周期统一决策回测V9公开仓库基线";
contract.strategyBaseline.baselineId = "unified-decision-v3-factors-v6-minute-sell-v9-public";
